...
def get_directory_specific(args) -> str:
    print("Calling get_directory_specific ...")
...
def main(args):
    ...
    # print(f"Getting directory specifics: {curr_directory_specific}")

    start_folder = f"start_{str(args.reuse_aml_seed)}" if args.reuse_aml else f"start_{str(args.seed_id)}"
    ...
    # print(f"Working on the automl_path: {automl_path}")


    for curr_al_iteration in range(1, args.al_max_iter + 1):
        print(f"Current AL iteration: {curr_al_iteration}")
        ...
        print(f"check_aml_path: {check_aml_path}")

        print(f"Does AML path exist? {os.path.exists(check_aml_path)}")
